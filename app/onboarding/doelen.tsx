...
export default function OnboardingDoelenScreen() {
  const { colors } = useTheme();
  const router = useRouter();
  const { profile, saveProfile, saveOnboarding } = useStore();

  const [selected, setSelected] = useState<AppDoel[]>([]);
  ...
  return (
    <SafeAreaView style={[s.safe, { backgroundColor: colors.bg }]}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Button title="Overslaan" onPress={handleSkip} variant="ghost" size="md" />
      </ScrollView>
    </SafeAreaView>
  );
}
